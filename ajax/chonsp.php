<?php
    ...
    if($result_check_contain->num_rows > 0){
        ...
        $sql_getgiaban = "SELECT gia_ban from sanpham where id_sp = ".$idsp."";
        $result_getgiaban = $con->query($sql_getgiaban);
        $row_getgiaban = $result_getgiaban->fetch_assoc();
        $sql = "INSERT INTO `thanhtoan_tam` (`idsp`, `sl_sp`, `dongia_sp`) VALUES ('$idsp', '$sl_sp', '".$row_getgiaban['gia_ban']."')";
        $con->query($sql);
    }
    ...
    if($result_thanhtoan_tam->num_rows > 0){
        ...
        while($row_thanhtoan_tam = $result_thanhtoan_tam->fetch_assoc()){
            $sql_show = "SELECT sp.id_sp, sp.ten_sp, ttt.dongia_sp, ttt.sl_sp from sanpham sp, thanhtoan_tam ttt where sp.id_sp = ".$row_thanhtoan_tam['idsp']." and sp.id_sp = ttt.idsp";
            ...
            echo "
                <tr>
                    ...
                    <td>".$row['ten_sp']."</td>
                    <td>".$row['sl_sp']."</td>
                    <td>".number_format($row['dongia_sp'], 0, '', ',')."</td>
                    <td>".number_format($row['dongia_sp']*$row['sl_sp'], 0, '', ',')."</td>
