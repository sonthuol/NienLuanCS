<?php
    ...
    if($result_getdata_thanhtoantt->num_rows > 0){
        while($row_getdata_thanhtoantt = $result_getdata_thanhtoantt->fetch_assoc()){
            ...
            $tt = $row_getdata_thanhtoantt['dongia_sp']*$row_getdata_thanhtoantt['sl_sp'];
            $sql_setchitiethd = "INSERT into chitiethoadon(id_cthd ,id_hd, id_sp, dongia, sl_sp, thanhtien)
            value (null, '".$idhd."', '".$row_getdata_thanhtoantt['idsp']."', '".$row_getdata_thanhtoantt['dongia_sp']."',  '".$row_getdata_thanhtoantt['sl_sp']."',  $tt)";
        ...
        while($row_thanhtoan_tam = $result_thanhtoan_tam->fetch_assoc()){
            ...
            $sql_capnhat = "UPDATE sanpham set sl_sp = '".$sl_capnhat."' where id_sp = '".$row_thanhtoan_tam['sl_sp']."'";
